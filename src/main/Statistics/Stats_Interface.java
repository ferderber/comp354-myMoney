...
public class Stats_Interface {
	...
	public List<String> ExtremeMonths(List<Transaction> list) {
		...
		temp = "Biggest Outflow is in the month of " + toMonth(MaxOutMonth) + " in " + MaxOutYear + "\n" + " Amount: $"
				+ transMaxOUT.getAmount() + "\n";
		array.add(temp);
		temp = "";

		int monthsDone = 0;
		int months = stat.getTotalMonths(list);
		ArrayList<Integer> year = stat.getYearList(list);
		...
		while (months != 0) {
			...
			MonthAvg = stat.getAverageByYearAndMonth(list, Cmonth, Cyear);

			if (MonthAvg != 0) {
				MonthMed = stat.getMedianByYearAndMonth(list, Cmonth, Cyear);
				MonthIN = stat.getAverageInByYearAndMonth(list, Cmonth, Cyear);
				MonthOUT = stat.getAverageOutByYearAndMonth(list, Cmonth, Cyear);
	...
	public List<Transaction> select(List<Transaction> list, String select) {
		String Temp = "";
		String Temp2 = "";
		...
		if (select.charAt(0) == '2' && select.charAt(1) == '0' && select.charAt(2) == '1') {
			...
			return stat.getTransactionByYear(list, trans);
		}

		if ((int) select.charAt(0) > 47 && (int) select.charAt(0) < 58) {
			trans = Integer.parseInt(select);
			return stat.getMonthsTransactions(list, trans);
		}
		...
		for (int i = 0; i < select.length(); i++) {
			if ((int) select.charAt(i) == 32) {
				...
				return stat.getTransactionByMonth(list, trans2, trans);
			}
		}
	...
	public List<String> displayStats(List<Transaction> allTransactions, String select, String Type) {
		Statistics stat = new Statistics();

		List<Transaction> list = select(allTransactions, select);
	...
	public List<String> displayRecurring(List<Transaction> allTransactions, String select, int times, String Type) {
		Statistics stat = new Statistics();
	...
	public BarChart displayYearStats(List<Transaction> list) {

		Statistics stat = new Statistics();

		List<String> Names = new ArrayList<String>();
		List<Integer> Years = stat.getYearList(list);
	...
	public static void main(String args[]) {
		...
		BarChart bc = inter.displayYearStats(allTransactions);

		list = inter.displayStats(allTransactions, "March", null);
